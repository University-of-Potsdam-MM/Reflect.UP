...
export class SelectModulePage extends AbstractPage implements OnInit {
  ...
  async presentTOS() {
    ...
    if (!agreeTOS && !tosPreVersion7) {
      ...
      this.alert.showAlert(
        {
          messageI18nKey: "templates.tos",
          headerI18nKey: "statusMessage.tos.title",
        },
        buttons
      );
    }
    ...
    for (let i = 0; i < filterResults.length; i++) {
      if (alreadyLoggedInSessions) {
        for (let j = 0; j < alreadyLoggedInSessions.length; j++) {
          if (
            filterResults[i].courseID === alreadyLoggedInSessions[j].courseID
          ) {
      ...
      if (!filterResults[i].isChecked) {
        filterResults[i].isChecked = false;
      }
  ...
  filterItems(searchTerm) {
    return this.moduleConfigList.filter(
      (item) =>
        item.faculty
          .toLowerCase()
          .replace(/-/g, "")
        ...
        item.courseID
          ...
          .slice(4, 6)
          .concat("/")
          .concat(item.courseID.slice(6, 8))
          .indexOf(searchTerm.toLowerCase()) > -1
    );
  }
  ...
  selectConfigs() {
    ...
    for (let i = 0; i < this.facultyCourses.length; i++) {
      if (
        this.facultyCourses[i].isChecked &&
        !this.facultyCourses[i].alreadyLoggedIn
      ) {
    ...
    for (let i = 0; i < this.moduleCourses.length; i++) {
      if (
        this.moduleCourses[i].isChecked &&
        !this.moduleCourses[i].alreadyLoggedIn
      ) {
  ...
  async presentPopover(myEvent) {
    const popover = await this.popoverCtrl.create({
      ...
      componentProps: {
        moduleConfigList: this.moduleConfigList,
