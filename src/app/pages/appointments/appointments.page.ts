...
export class AppointmentsPage extends AbstractPage implements OnInit {
  ...
  ngOnInit() {
    if (this.translate.currentLang === "en") {
      this.optionsBasic.monthPickerFormat = [
        "JAN",
        "FEB",
        "MAR",
        "APR",
      ...
      if (online || !ionRefresh) {
        ...
        const loop = dLoop(this.sessions, (itm, idx, fin) => {
          if (itm && !itm.isHidden) {
            ...
            this.appointm
              .getAppointments(config, itm.token, ionRefresh)
              .subscribe(
                async (appointConfig: AppointConfig) => {
                  this.logger.debug(
                    "initEvents()",
                  ...
                  if (appointConfig && appointConfig.events) {
                    ...
                    const scheduledArray = await this.storage.get(
                      "scheduledEvents"
                    );
                    let notificationID;
                    for (const event of appointConfig.events) {
                      if (event.modulename !== "feedback") {
                        ...
                        if (scheduledArray) {
                          foundScheduled = scheduledArray.find(
        ...
        loop.then(() => {
          ...
          this.tmpEventList = tmpEventArray.sort(
            (a: EventObject, b: EventObject) => a.timestart - b.timestart
          );
          this.isLoaded = true;
  ...
  checkEventDates() {
    ...
    for (i = 0; i < this.tmpEventList.length; i++) {
      ...
      endDate = moment(
        (this.tmpEventList[i].timestart + this.tmpEventList[i].timeduration) *
          1000
      );
      ...
      if (moment(beginDate).isSameOrBefore(currentDate, "day")) {
        ...
        moment(beginDate).isSameOrBefore(tommorowDate, "day") &&
        moment(endDate).isSameOrAfter(tommorowDate, "day")
      ) {
        // tomorrow
        ...
        moment(beginDate).isSameOrBefore(currentDate, "week") &&
        moment(endDate).isSameOrAfter(currentDate, "week")
      ) {
        // this week
  ...
  pickDate($event) {
    ...
    for (i = 0; i < this.tmpEventList.length; i++) {
      ...
      if (
        moment(beginDate).isSame($event, "day") ||
        (moment(beginDate).isBefore($event) &&
          moment(endDate).isSameOrAfter($event))
      ) {
  ...
  visibilityChanged(eventID) {
    if (!this.hiddenEvent[eventID]) {
      ...
      this.hiddenEvent[eventID] = false;
    }
  }
