...
export class PushService {
  ...
  subscribeToPush(registrationID, config: IModuleConfig, fin?) {
    const logger: Logger = this.loggingService.getLogger(
      "[/push-service-subscribe]"
    );

    logger.debug(
      "subscribeToPush()",
      "subscribing to push service for " + config.courseID
    );
    ...
    headerAppName = headerAppName.concat(
      config.courseID.replace(/-/g, "").toLowerCase()
    );
    ...
    this.nativeHTTP
      .post(url_subscribe, myData, myHeaders)
      .then(() => {
        logger.debug(
          "subscribeToPush()",
          "successfully contacted the push server"
        );
      ...
      .catch((error) => {
        logger.error(
          "subscribeToPush()",
          "error while contacting the push server",
          error
        );
  ...
  unsubscribeToPush(config: IModuleConfig) {
    const logger: Logger = this.loggingService.getLogger(
      "[/push-service-unsubscribe]"
    );

    // unsubscribe from the AirNotifier push service
    const url_unsubscribe =
      config.pushDetails.uniqushUrl.concat("tokens/") +
    ...
    this.nativeHTTP.setDataSerializer("json");
    this.nativeHTTP
      .delete(url_unsubscribe, {}, myHeaders)
      .then(() => {
        logger.debug(
          "unsubscribeToPush()",
          "successfully unsubscribed from the push server"
        );
      })
      .catch((error) => {
        logger.error(
  ...
  async registerPushService() {
      ...
      await this.push
        ...
        .then(
          () => {
            logger.debug(
              "registerPushService()",
              "successfully created push channel"
            );
          },
          (error) => {
            logger.error(
              "registerPushService()",
              "push error on channel creation",
              error
            );
          }
        );
    ...
    pushObject.on("notification").subscribe(
      async (data) => {
        ...
        logger.debug(
          "registerPushService() on(notification)",
        ...
        if (this.platform.is("ios")) {
          pushObject.finish().catch((error) => {
            logger.error(
              "registerPushService() finish()",
              "error while processing background push",
              error
            );
          });
        }
    ...
    pushObject.on("registration").subscribe(
      async (data) => {
        if (data.registrationId.length === 0) {
          logger.error(
            "registerPushService() on(registration)",
            "push registrationID is empty",
            data
          );
        } else {
          ...
          if (sessions) {
            ...
            logger.debug(
              "registerPushService() on(registration)",
              "user is not logged in; cant subscribe to push notifications"
            );
          }
        }
      },
