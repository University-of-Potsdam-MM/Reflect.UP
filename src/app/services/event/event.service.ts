...
export class EventService {
  ...
  public getAppointments(
    ...
    const params: HttpParams = new HttpParams()
      ...
      .append(
        "options[timestart]",
        Math.floor(today.getTime() / 1000).toString()
      )
      .append(
        "options[timeend]",
        Math.floor(oneYearLater.getTime() / 1000).toString()
      )
    ...
    const request = this.http.get<AppointConfig>(url, {
      headers: headers,
      params: params,
    });

    if (forceReload) {
      this.cache.removeItem("cachedEvents" + courseID);
    }
