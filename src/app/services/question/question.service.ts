...
export class QuestionService {
  ...
  public getQuestions(
    ...
    const request = this.http.get<QuestionConfig>(url, {
      headers: headers,
      params: params,
    });

    if (forceReload) {
      this.cache.removeItem("cachedQuestions" + courseID);
    }
  ...
  public getAnsweredQuestions(
    ...
    if (forceReload) {
      this.cache.removeItem("cachedCompletedQuestions" + courseID);
    }

    return this.cache.loadFromObservable(
      "cachedCompletedQuestions" + courseID,
      request
    );
  }
