    ...
    return new Promise<void>((resolve, reject) => {
      this.http.get(config_url).toPromise().then(
        (serverConfig: IModuleConfig[]) => {
          logger.debug('load()', 'successfully fetched remote config', serverConfig);

          this.http.get(uri).subscribe((localConfig: IModuleConfig[]) => {
            logger.debug('load()', 'successfully loaded local config', localConfig);
            if (serverConfig[0].appVersion >= localConfig[0].appVersion) {
              ...
              if (serverConfig[0].appVersion > localConfig[0].appVersion) {
                ...
                logger.debug('load()', 'server has a newer app version -- update available');
              }
        ...
        logger.error('load()', 'error while fetching remote config', errorServer);
        this.http.get(uri).subscribe((localConfig: IModuleConfig[]) => {
          logger.debug('load()', 'successfully loaded local config', localConfig);
