...
export class AppComponent {
  ...
  async oldVersionCompatibility() {
    ...
    if (hiddenCards === ["-1"]) {
      this.storage.remove("hiddenCards");
    }
    if (scheduledEvents === ["-1"]) {
      this.storage.remove("scheduledEvents");
    }
  }
    ...
    if (this.courseSessions) {
      for (let i = 0; i < this.courseSessions.length; i++) {
        const config: IModuleConfig = this.configService.getConfigById(
          this.courseSessions[i].courseID
        );
  ...
  enrollSelf(config: IModuleConfig, token) {
    ...
    this.http
      .get(moodleAccessPoint, { headers: headers, params: params })
      .subscribe(
        (response) => {
          this.logger.debug(
            "enrollSelf()",
            "successfully enrolled in course " + config.courseID,
            response
          );
        },
        (error) => {
          this.logger.error(
            "enrollSelf()",
            "error while enrolling in course " + config.courseID,
            error
          );
        }
      );
  ...
  async initializeTranslate() {
    ...
    this.logger.debug(
      "initializeTranslate()",
      "language is set to " + this.translate.currentLang
    );
  }
  ...
  async initializeMenu() {
    this.pagesInMenu = [
      ...
      {
        title: "pageHeader.appointmentsPage_2",
        pageName: "/appointments",
        icon: "calendar",
      },
      {
        title: "pageHeader.questionsPage",
        pageName: "/questions",
        icon: "create",
      },
      {
        title: "pageHeader.contactsPage",
        pageName: "/contacts",
        icon: "people-circle",
      },
      {
        title: "pageHeader.feedbackPage",
        pageName: "/feedback",
        icon: "document-text",
      },
      {
        title: "pageHeader.pushMessagesPage",
        pageName: "/push-messages",
        icon: "chatbubbles",
      },
    ];
    ...
    if (this.courseSessions) {
      for (const session of this.courseSessions) {
        ...
        if (config.mintEnabled) {
          this.pagesInMenu.push({
            title: "pageHeader.mintPage",
            pageName: "/mint",
            icon: "analytics",
            url: config.mintUrl,
          });
  ...
  private openWithSafariOrChrome(url: string) {
    this.safariOrChrome
      ...
      .subscribe(
        (result) => {
          this.logger.debug("openWithSafariOrChrome", result);
        },
        (error) => {
          this.logger.error("openWithSafariOrChrome", error);
        }
      );
  }
}
